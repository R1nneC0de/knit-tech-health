    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-3">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    {item.product?.category && (
                      <span className="text-xs text-brand-teal-500">{item.product.category.name}</span>
                    )}
        ...
        <div className="h-fit rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <Link
            href="/checkout"
            className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-brand-teal-500 px-6 py-3 font-semibold text-white transition hover:bg-brand-teal-600"
          >
            Proceed to Checkout
          ...
          <Link
            href="/shop-medical"
            className="mt-3 block text-center text-sm text-brand-blue-700 hover:text-brand-teal-500"
          >
            Continue Shopping
          </Link>
        </div>
